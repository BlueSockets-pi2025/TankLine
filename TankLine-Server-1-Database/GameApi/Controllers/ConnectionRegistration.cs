...
public class ConnectionRegistrationController : Controller
{
    ...
    public IActionResult Login([FromBody] LoginRequest loginRequest)
    {
        ...
        Response.Cookies.Append("RefreshToken", refreshToken, new CookieOptions
        {
            ...
            Secure = true,
            SameSite = SameSiteMode.Strict,
            Expires = user.RefreshTokenExpiration // Use DB expiration
        });
    ...
    public IActionResult AutoLogin()
    {
        ...
        if (user == null || string.IsNullOrEmpty(user.RefreshTokenHash))
            return Unauthorized("User not found or no refresh token saved.");
        ...
        return Ok("Auto-login successful");
    }
    ...
    public IActionResult RefreshToken()
    {
        ...
        if (!TokenService.VerifyRefreshToken(refreshToken, user.RefreshTokenHash))
        {
            return BadRequest("Invalid refresh token (not verified).");
        }
        ...
        // Generate a new JWT token (access-token)
    ...
    public IActionResult RefreshRefreshToken()
    {
        var refreshToken = Request.Cookies["RefreshToken"];
        ...
        var newRefreshToken = TokenService.GenerateRefreshToken(user.Username);
        ...
        return Ok("Refresh token refreshed successfully.");
    }

    [HttpPost("set-refresh-cookie")]
    public IActionResult SetRefreshCookie([FromBody] RefreshTokenRequest request)
    {
        if (string.IsNullOrEmpty(request?.RefreshToken))
            return BadRequest("Missing refresh token.");

        Console.WriteLine($"Received refresh token: {request.RefreshToken}");
        ...
        var user = _context.UserAccounts.FirstOrDefault(u => u.Username == username);
        if (user == null || string.IsNullOrEmpty(user.RefreshTokenHash))
            return BadRequest("Invalid refresh token (empty).");

        if (!TokenService.VerifyRefreshToken(request.RefreshToken, user.RefreshTokenHash))
            return BadRequest("Invalid refresh token (not verified).");
        
        if (user.RefreshTokenExpiration < DateTime.UtcNow)
            return BadRequest("Refresh token expired.");

        Response.Cookies.Append("RefreshToken", request.RefreshToken, new CookieOptions
        {
            HttpOnly = true,
            Secure = true,
            SameSite = SameSiteMode.Strict,
        ...
        Console.WriteLine($"Refresh token set for user: {username}");

        return Ok(new { Message = "Cookie restored." });
    }

    [HttpPost("logout")]
    public IActionResult Logout()
    {
        var refreshToken = Request.Cookies["RefreshToken"]; // Get the refresh token from the cookie
        // Delete the cookies 
    ...
    public IActionResult GetRefreshTokenExpiration()
    {
        var refreshToken = Request.Cookies["RefreshToken"];
        if (string.IsNullOrEmpty(refreshToken)) return Unauthorized("Missing token.");

        var parts = refreshToken.Split(':');
        if (parts.Length != 2) return BadRequest("Invalid token format.");
        var username = parts[0];

        var user = _context.UserAccounts.FirstOrDefault(u => u.Username == username);
        if (user == null || user.RefreshTokenExpiration == null)
            return BadRequest("User not found or no refresh token.");

        return Ok(new { ExpiresAt = user.RefreshTokenExpiration });
    }

    [HttpPost("disconnect")]
    public IActionResult DisconnectOnQuit()
    {
        var refreshToken = Request.Cookies["RefreshToken"];
        if (string.IsNullOrEmpty(refreshToken))
        ...
        var user = _context.UserAccounts.FirstOrDefault(u => u.Username == username);
        if (user == null || string.IsNullOrEmpty(user.RefreshTokenHash))
            return NotFound("User not found or no refresh token stored.");

        if (!TokenService.VerifyRefreshToken(refreshToken, user.RefreshTokenHash))
            return Unauthorized("Invalid refresh token.");

        // Logical disconnection to allow reconnection (without loggin-out):
        user.IsLoggedIn = false;
        user.LastActivity = DateTime.UtcNow;
        ...
        return Ok("Disconnection successful.");
    }
